...
function formField(field, record = {}) {
  if (field.createOnly && record.id !== undefined) return '';
  const input = field.type === 'textarea'
    ? `<textarea name="${field.name}" ${field.required ? 'required' : ''} placeholder="Informe ${field.label.toLowerCase()}">${escapeHtml(record[field.name] ?? '')}</textarea>`
    : `<input name="${field.name}" type="${field.type}" value="${escapeHtml(field.type === 'datetime-local' && record[field.name] ? String(record[field.name]).slice(0, 16) : record[field.name] ?? '')}" ${field.required ? 'required' : ''} ${field.pattern ? `pattern="${field.pattern}"` : ''} ${field.maxlength ? `maxlength="${field.maxlength}"` : ''} placeholder="Informe ${field.label.toLowerCase()}" />`;
  return `<label class="form-field"><span>${field.label}${field.required ? ' <b>*</b>' : ''}</span>${input}${field.hint ? `<small>${field.hint}</small>` : ''}</label>`;
}
...
export function openForm(key, record, onSaved) {
  const config = RESOURCE_CONFIG[key]; const editing = Boolean(record);
  showModal(`${editing ? 'Editar' : 'Cadastrar'} ${config.singular}`, `<form id="record-form" class="record-form"><div class="form-grid">${config.fields.map(field => formField(field, record)).join('')}</div><div class="modal-actions"><button type="button" class="secondary-button" data-modal-close>Cancelar</button><button class="primary-button" type="submit">${editing ? 'Salvar alterações' : 'Cadastrar'}</button></div></form>`);
  document.querySelector('#record-form').addEventListener('submit', async event => {
    event.preventDefault(); const form = event.currentTarget;
    if (!form.reportValidity()) return;
    const data = Object.fromEntries(new FormData(form));
    config.fields.forEach(field => { if (field.type === 'number' && data[field.name] !== undefined) data[field.name] = Number(data[field.name]); if (field.transform === 'uppercase' && data[field.name]) data[field.name] = data[field.name].toUpperCase(); });
